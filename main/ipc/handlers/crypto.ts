 ...
 */
async function getKeys(event: IpcMainInvokeEvent): Promise<CryptoResult<CryptoKey[]>> {
  try {
 ...
 */
async function getCertificates(event: IpcMainInvokeEvent): Promise<CryptoResult<Certificate[]>> {
  try {
    ...
    if (nodeOneCore.initialized && nodeOneCore.ownerId) {
      try {
        ...
        if ((nodeOneCore as any).iomManager) {
          try {
            const iomGroup = await (nodeOneCore as any).iomManager.iomGroup();
            if (iomGroup) {
              certificates.push({
                id: 'iom-cert',
 ...
 */
async function exportCryptoObject(event: IpcMainInvokeEvent, request: ExportRequest): Promise<CryptoResult<ExportData>> {
