...
import ipcController from '../controller.js';
...
// BlobDescriptor cache instance (imported as singleton)
const blobDescriptorCache = BlobDescriptorCache
...
const chatHandlers = {
  ...
  async sendMessage(event: IpcMainInvokeEvent, { conversationId, text, attachments = [] }: SendMessageParams): Promise<IpcResponse> {
    ...
    try {
      ...
      */
      ...
      ipcController.sendUpdate('chat:newMessages', {
        conversationId,
        messages: []
      });
        ...
        if (msg.attachments && msg.attachments.length > 0) {
          ...
          for (const attachmentHash of msg.attachments) {
            try {
              ...
              const blobDescriptor = await blobDescriptorCache.get(attachmentHash)
    ...
    try {
      ...
      if (nodeOneCore.initialized && nodeOneCore.channelManager) {
        ...
        for (const channel of channels) {
          ...
          if (channel.id === 'contacts' || channel.id === 'ai-subjects') continue
  ...
  async addParticipants(event: IpcMainInvokeEvent, { conversationId, participantIds }: AddParticipantsParams): Promise<IpcResponse> {
    ...
    try {
      ...
      const hasGroup = nodeOneCore.topicGroupManager.hasConversationGroup(conversationId)
      ...
      if (isLegacyP2P && !hasGroup) {
        ...
        await nodeOneCore.topicGroupManager.createGroupTopic(
          groupName,
          newGroupId,
          uniqueParticipants as SHA256IdHash<Person>[]
        )
      ...
      await nodeOneCore.topicGroupManager.addParticipantsToTopic(conversationId, validParticipantIds as SHA256IdHash<Person>[])
