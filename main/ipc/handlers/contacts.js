...
export function registerContactHandlers() {
    ...
    if (nodeOneCore.quicTransport?.leuteModel) {
        nodeOneCore.quicTransport.leuteModel.on('pending-contact', (data) => {
            // Send to all windows
        ...
        nodeOneCore.quicTransport.leuteModel.on('contact-accepted', (data) => {
            const { BrowserWindow } = require('electron');
            BrowserWindow.getAllWindows().forEach((window) => {
                window.webContents.send('contacts:accepted', data);
            });
        });
        nodeOneCore.quicTransport.leuteModel.on('dedicated-vc-received', (data) => {
            const { BrowserWindow } = require('electron');
            BrowserWindow.getAllWindows().forEach((window) => {
