 ...
 */
function getNodeOneCoreInstance() {
...
${String(conversationText).substring(0, 3000)}`;
        ...
        for (const subject of subjects.slice(0, 5)) {
            await model.createSubject(topicId, subject.keywords, subject.keywords.join('+'), subject.description, 0.8);
        }
...
${String(conversationText).substring(0, 3000)}`;
        ...
        // Create summary
        const summary = await model.createSummary(topicId, 1, summaryResponse, [], 'AI-generated analysis', null);
 ...
 */
export async function getKeywords(event, { topicId, limit = 100 }) {
    console.log('[TopicAnalysis] Getting keywords for topic:', topicId);
    try {
        const model = await initializeModel();
...
export async function getConversationRestartContext(event, { topicId }) {
    ...
    try {
        ...
        if (subjects.length > 0) {
            ...
            const subjectDescriptions = activeSubjects.map((s) => `- ${s.keywordCombination}: ${s.description || 'Active subject'}`).join('\n');
    ...
    try {
        ...
        if (autoGenerate && !content) {
            ...
            if (llmManager && modelId) {
                ...
                const messagesResponse = await chatHandlers.default.getMessages(event, {
...
${String(conversationText).substring(0, 3000)}`;
        ...
        const newSummary = await model.createSummary(topicId, newVersion, summaryContent || content || '', [], changeReason || 'Manual update', currentSummary ? currentSummary.id : null);
        return {
            success: true,
            data: {
                summary: newSummary
...
// Keyword extraction cache: Map<topicId, { messageCount, keywords, timestamp }>
const keywordCache = new Map();
const KEYWORD_CACHE_TTL = 30000; // 30 seconds
...
export async function extractConversationKeywords(event, { topicId, messages = [], maxKeywords = 15 }) {
    ...
    try {
        ...
        if (!messages || messages.length === 0) {
            const chatHandlers = await import('./chat.js');
            const messagesResponse = await chatHandlers.default.getMessages(event, { conversationId: topicId });
        ...
        // Check cache first
        const cached = keywordCache.get(topicId);
        const now = Date.now();
        if (cached && cached.messageCount === messages.length && (now - cached.timestamp) < KEYWORD_CACHE_TTL) {
            console.log('[TopicAnalysis] Returning cached keywords for topic:', topicId);
            return {
                success: true,
                data: {
