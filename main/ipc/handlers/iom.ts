 ...
 */
async function getIOMInstances(event: IpcMainInvokeEvent): Promise<Instance[]> {
  try {
    ...
    if (nodeProvisioning.isProvisioned()) {
      ...
      const nodeState: { [key: string]: any } = {}

      // Get Node instance info
      const nodeInfo: Instance = {
 ...
 */
async function getStorageInfo(type: string): Promise<StorageInfo> {
  try {
    if (type === 'node') {
      ...
      try {
        ...
        if (process.platform === 'darwin' || process.platform === 'linux') {
          ...
          const freeMatch = String(wmicOutput).match(/FreeSpace=(\d+)/)
          const sizeMatch = String(wmicOutput).match(/Size=(\d+)/)
 ...
 */
async function getReplicationEvents(event: IpcMainInvokeEvent): Promise<SyncEvent[]> {
 ...
 */
function addReplicationEvent(event: Omit<SyncEvent, 'id' | 'timestamp'>): SyncEvent {
 ...
 * Get data statistics
 */
async function getDataStats(event?: IpcMainInvokeEvent): Promise<DataStats> {
  try {
...
function startMonitoring(): void {
  ...
  chumMonitor.on('connection:open', (connId: string) => {
    addReplicationEvent({
      type: 'sync-started',
      source: 'node',
      target: 'remote',
  ...
  chumMonitor.on('connection:closed', (data: any) => {
    addReplicationEvent({
      type: 'sync-completed',
      source: 'node',
      target: 'remote',
  ...
  chumMonitor.on('connection:error', (data: any) => {
    addReplicationEvent({
      type: 'sync-failed',
      source: 'node',
      target: 'remote',
      details: (data.error as Error)?.message || 'Connection error',
      status: 'error'
    })
  })
  ...
  chumMonitor.on('sync:progress', (data: any) => {
    const now = Date.now()
  ...
  chumMonitor.on('sync:completed', (data: any) => {
    addReplicationEvent({
      type: 'sync-completed',
      source: 'node',
      target: 'remote',
 ...
 */
async function updateBrowserStorage(event: IpcMainInvokeEvent, storageInfo: StorageInfo): Promise<{ success: boolean }> {
 ...
 */
async function updateDataStats(event: IpcMainInvokeEvent, stats: DataStats): Promise<{ success: boolean }> {
 ...
 */
async function acceptPairingInvitation(event: IpcMainInvokeEvent, invitationUrl: string): Promise<{ success: boolean; message?: string; error?: string }> {
  try {
    // Check if Node instance is provisioned
    if (!nodeProvisioning.isProvisioned()) {
      return {
    ...
    const fragment = String(invitationUrl).substring(hashIndex + 1)
    ...
    console.log('[IOM] Accepting invitation with token:', String(token).substring(0, 20) + '...')
    ...
    addReplicationEvent({
      ...
      source: 'node',
      target: 'remote',
      details: (error as Error).message || 'Failed to accept invitation',
      status: 'error'
    })
 ...
 */
async function createPairingInvitation(event: IpcMainInvokeEvent): Promise<{ success: boolean; invitation?: { url: string; token: string }; error?: string }> {
  try {
    // Check if Node instance is provisioned
    if (!nodeProvisioning.isProvisioned()) {
      return {
    ...
    try {
      ...
      if ((nodeOneCore.connectionsModel as any)?.leuteConnectionsModule?.connectionRouteManager?.catchAllRoutes) {
        const catchAllRoutes = (nodeOneCore.connectionsModel as any).leuteConnectionsModule.connectionRouteManager.catchAllRoutes
        ...
        if (!hasMatchingKey) {
          ...
          if ((nodeOneCore.connectionsModel as any)?.leuteConnectionsModule?.myPublicKeyToInstanceInfoMap) {
            const instanceMap = (nodeOneCore.connectionsModel as any).leuteConnectionsModule.myPublicKeyToInstanceInfoMap
      ...
      return {
        success: false,
        error: `Failed to create invitation: ${(error as Error).message}`
      }
    }
