...
const chatHandlers = {
    ...
    async sendMessage(event, { conversationId, text, attachments = [] }) {
        ...
        try {
            ...
            if (attachments && attachments.length > 0) {
                ...
                await topicRoom.sendMessageWithAttachmentAsHash(text || '', // Ensure text is not undefined
                attachmentHashes, undefined, // author - undefined means use my main identity
                channelOwner);
            }
            else {
    ...
    async getMessages(event, { conversationId, limit = 50, offset = 0 }) {
        ...
        try {
            ...
            if (conversationId.includes('<->')) {
                ...
                console.log(`[ChatHandler] P2P conversation - Found ${allChannels.length} channels:`, allChannels.map((c) => ({
                    id: c.id,
        ...
        try {
            ...
            if (nodeOneCore.topicGroupManager) {
                try {
                    ...
                    await nodeOneCore.topicGroupManager.createGroupTopic(conversation.name, conversation.id, validParticipantIds);
    ...
    async getConversations(event, { limit = 20, offset = 0 } = {}) {
        ...
        try {
            ...
            if (nodeOneCore.initialized && nodeOneCore.channelManager) {
                ...
                const channelDebug = {}(channels).forEach((ch) => {
                    if (!channelDebug[ch.id])
                        channelDebug[ch.id] = [];
                ...
                if (nodeOneCore.aiAssistantModel) {
                    ...
                    conversations.forEach((conv) => {
                        ...
                        conv.hasAIParticipant = conv.participants?.some((participantId) => aiContacts.some((contact) => contact.personId === participantId)) || false;
    ...
    async getConversation(event, { conversationId }) {
        ...
        try {
            ...
            if (nodeOneCore.aiAssistantModel) {
                const aiContacts = nodeOneCore.aiAssistantModel.getAllContacts()(conversation).isAITopic = nodeOneCore.aiAssistantModel.isAITopic(conversation.id);
                // Check if any participants are AI
    ...
    async addParticipants(event, { conversationId, participantIds }) {
        ...
        try {
            ...
            if (conversationId.includes('<->') || conversationId === 'lama') {
                ...
                await nodeOneCore.topicGroupManager.createGroupTopic(groupName, newGroupId, uniqueParticipants);
                    ...
                    if (p2pMessages && p2pMessages.length > 0) {
                        ...
                        for (const message of p2pMessages) {
                            if (message.data && message.data.text) {
                                ...
                                await nodeOneCore.topicRoom.sendMessage(message.data.text, sender // Post as the original sender
                                );
                            }
                        }
    ...
    async exportMessageCredential(event, { messageId }) {
        ...
        try {
            ...
            if (!messageAssertionManager && nodeOneCore.trust && nodeOneCore.leuteModel) {
                messageAssertionManager = new MessageAssertionManager(nodeOneCore.trust, nodeOneCore.leuteModel);
            }
            if (!messageAssertionManager) {
    ...
    async verifyMessageAssertion(event, { certificateHash, messageHash }) {
        ...
        try {
            ...
            const verification = await messageAssertionManager.verifyMessageAssertion(certificateHash, messageHash);
            return {
                success: true,
                data: verification
            };
