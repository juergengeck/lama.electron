 ...
 */
async function exportFile(event, { content, filename, filters }) {
    try {
 ...
 */
async function exportFileAuto(event, { content, filename, mimeType }) {
    try {
 ...
 */
async function exportMessage(event, { format, content, metadata }) {
    try {
        console.log('[Export] exportMessage called:', { format, contentLength: content.length });
        let filename, fileContent, filters;
        switch (format) {
            case 'markdown':
 ...
 */
async function exportHtmlWithMicrodata(event, { topicId, format, options = {} }) {
    try {
        ...
        const result = await Promise.race([
            exportPromise,
            new Promise((_, reject) => setTimeout(() => reject(new Error('Export timeout after 30 seconds')), timeout))
        ]);
        return result;
    }
    catch (error) {
 ...
 */
function validateExportRequest({ topicId, format, options = {} }) {
    // Validate topicId
 ...
 */
async function performExport(topicId, options, services) {
    ...
    try {
        ...
        const processedMessages = await processMessagesWithImplode(messages, options, implodeWrapper);
        // Check timeout
 ...
 */
async function getMessagesFromTopic(topicId, options) {
 ...
 */
async function processMessagesWithImplode(messages, options, implodeWrapper) {
 ...
 */
async function generateMetadata(topicId, messages, options) {
 ...
 */
function generateEmptyConversationHTML(topicId, options) {
