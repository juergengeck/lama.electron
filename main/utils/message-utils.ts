 ...
 */
function createUserMessage(text: any, sender: any, attachments = []): any {
  return {
    $type$: 'ChatMessage',
    text,
    sender,
 ...
 */
function isAIMessage(message: any, aiPersonIds: any): any {
  if (!message || !message.sender) return false;
  const senderId = message.sender.toString();
  return aiPersonIds.some((aiId: any) => aiId.toString() === senderId);
}
