...
class Summary {
   ...
   */
  createNewVersion(newContent, changeReason = '') {
   ...
   */
  isSignificantChange(newContent, newSubjects = [], threshold = 0.2) {
    ...
    const added = newSubjects.filter(s => !oldSubjectSet.has(s));
    const removed = this.subjects.filter(s => !newSubjectSet.has(s));
