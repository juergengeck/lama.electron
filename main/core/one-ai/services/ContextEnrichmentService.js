...
export class ContextEnrichmentService {
    ...
    async getImplicitContext(topicId, currentMessage) {
        try {
            ...
            const resonantPatterns = await this.findResonantPatterns(messageKeywords, topicKeywords);
    ...
    async searchByKeyword(keyword) {
        ...
        for (const channel of channels) {
            ...
            const matches = keywords.filter((k) => k.term.toLowerCase().includes(keyword.toLowerCase()));
            // Get associated subjects
            for (const match of matches) {
                const subjects = await this.channelManager.getObjectsWithType('Subject', {
                    channelId: channel.id
                });
                const relatedSubjects = subjects.filter((s) => s.keywords.some((k) => k.toLowerCase() === match.term.toLowerCase()));
    ...
    calculateResonance(obj, historicalKeywords) {
        ...
        for (const term of objectTerms) {
            if (historicalTerms.includes(term)) {
                matches++;
            }
        }
