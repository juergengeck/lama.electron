...
class RealTimeKeywordExtractor {
    ...
    isMeaningfulWord(word) {
        ...
        if (consonantRatio > 0.75 && word.length > 5)
            return false;
        return true;
    }
    /**
     ...
     */
    extractFromMessages(messages, maxKeywords = 15) {
        ...
        for (const message of messages) {
            ...
            for (const keyword of uniqueInMessage) {
                aggregatedFreq[keyword] = (aggregatedFreq[keyword] || 0) + 1;
                messageCount[keyword] = (messageCount[keyword] || 0) + 1;
            }
        }
        // Calculate TF-IDF-like score (term frequency * inverse document frequency)
        const scoredKeywords = Object.entries(aggregatedFreq).map(([word, freq]) => {
            const tf = freq;
            const df = messageCount[word];
     ...
     */
    mergeKeywords(existingKeywords, newText, maxKeywords = 15) {
