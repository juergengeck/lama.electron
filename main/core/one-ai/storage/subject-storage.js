...
class SubjectStorage {
  ...
  async get(subjectIdHash) {
    ...
    try {
      ...
      if (result && result.obj) {
        return Subject.fromObject(result.obj);
      }
    } catch (error) {
   ...
   */
  async getByTopic(topicId, includeArchived = false) {
    if (!this.nodeOneCore?.initialized) {
      throw new Error('ONE.core not initialized');
    }
   ...
   * Merge two subjects
   */
  async merge(subjectId1, subjectId2, newKeywords = []) {
   ...
   */
  async findByKeywords(keywords, topicId = null) {
    const normalizedKeywords = keywords.map(k => k.toLowerCase());
    const subjects = [];

    const prefix = topicId ? `${this.storagePrefix}${topicId}:` : this.storagePrefix;
    const allKeys = await this.nodeOneCore.listObjects(prefix);
