...
export class TopicExporter {
  ...
  public attestationManager: any;

  constructor(channelManager: any, attestationManager: any) {
   ...
   */
  async exportTopicWithAttestations(params: any): Promise<any> {
   ...
   * @private
   */
  exportAsJSON(topicData: any, messages: any, attestationsByMessage: any): any {
    const exportObj = {
      $type$: 'AuditedTopic',
   ...
   * @private
   */
  exportAsHTML(topicData: any, messages: any, attestationsByMessage: any): any {
...
<body>
  ...
  ${messages.map((msg: any) => this.renderMessage(msg, attestationsByMessage.get(msg.hash) || [])).join('\n')}
</body>
</html>`;
   ...
   * @private
   */
  renderMessage(message: any, attestations: any): any {
    return `
  <div itemscope itemtype="https://one.core/Message" class="message">
    ...
    ${attestations.map((att: any) => this.renderAttestation(att)).join('\n')}
  </div>`;
  }
   ...
   * @private
   */
  renderAttestation(attestation: any): any {
    return `
   ...
   * @private
   */
  exportAsMicrodata(topicData: any, messages: any, attestationsByMessage: any): any {
  ...
  <div itemprop="message" itemscope itemtype="https://one.core/Message">
    ...
    ${(attestationsByMessage.get(msg.hash) || []).map((att: any) => `
   ...
   * @private
   */
  async getTopicData(topicId: any): Promise<any> {
   ...
   * @private
   */
  async getTopicMessages(topicId: any): Promise<any> {
   ...
   * @private
   */
  groupAttestationsByMessage(attestations: any): any {
   ...
   * @private
   */
  countTotalAttestations(attestationsByMessage: any): any {
    let count = 0;
