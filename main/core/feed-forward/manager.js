...
class FeedForwardManager extends EventEmitter {
    ...
    async createSupply(params) {
        try {
            ...
            const result = await storeVersionedObject(supply);
    ...
    async createDemand(params) {
        try {
            ...
            // Store in ONE.core (cast to any to bypass TypeScript type checking for now)
            const { storeVersionedObject } = await import('@refinio/one.core/lib/storage-versioned-objects.js');
            const result = await storeVersionedObject(demand);
        ...
        try {
            ...
            if (!demand) {
                ...
                try {
                    ...
                    demand = await getObjectByIdHash(params.demandHash);
                    if (demand) {
            ...
            for (const [supplyHash, supply] of this.supplyCache.entries()) {
                ...
                const matchedKeywords = supply.keywords.filter((hash) => demand.keywords.includes(hash));
            ...
            for (const match of limitedMatches) {
                ...
                await storeUnversionedObject(matchRecord);
            }
     ...
     */
    async getTrustScoreForParticipant(participantId) {
        try {
     ...
     */
    async getTrustScore(participantId) {
        try {
     ...
     */
    async calculateTrustComponents(participantId) {
     ...
     */
    calculateOverallTrustScore(components) {
     ...
     */
    async storeTrustScore(participantId, score, components) {
        const trustScore = {
            $type$: 'TrustScore',
            participantId,
            score,
        ...
        const { storeVersionedObject } = await import('@refinio/one.core/lib/storage-versioned-objects.js');
        await storeVersionedObject(trustScore);
    }
    /**
    ...
    async updateTrust(params) {
        try {
            ...
            const { storeVersionedObject } = await import('@refinio/one.core/lib/storage-versioned-objects.js');
            await storeVersionedObject(updatedTrustScore);
