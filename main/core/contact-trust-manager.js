...
class ContactTrustManager extends EventEmitter {
  ...
  async createDiscoveryVC(credential, peerId, discoverySource) {
    ...
    const signature = await sign(JSON.stringify(discoveryVC), keys.privateSignKey)
  ...
  async createAcceptanceVC(personId, options = {}) {
    ...
    const { sign } = await import('@refinio/one.core/lib/crypto/sign.js')
    const signature = await sign(JSON.stringify(acceptanceVC), keys.privateSignKey)
  ...
  async storeContactWithVC(credential, discoveryVC) {
    ...
    const someone = await SomeoneModel.constructWithNewSomeone(someoneData)
  ...
  async createBlockVC(personId, reason) {
    ...
    // Sign it
    const { sign } = await import('@refinio/one.core/lib/crypto/sign.js')
    const signature = await sign(JSON.stringify(blockVC), keys.privateSignKey)
