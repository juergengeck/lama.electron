...
class ContactTrustManager extends EventEmitter {
     ...
     */
    async createDiscoveryVC(credential, peerId, discoverySource) {
        ...
        const signature = await sign(new TextEncoder().encode(JSON.stringify(discoveryVC)), keys.privateSignKey);
        return {
            ...discoveryVC,
            signature: signature
        };
     ...
     */
    async createAcceptanceVC(personId, options = {}) {
        const { getInstanceIdHash, getInstanceOwnerIdHash } = await import('@refinio/one.core/lib/instance.js');
        const { getDefaultKeys } = await import('@refinio/one.core/lib/keychain/keychain.js');
        const instanceId = getInstanceIdHash();
        const myPersonId = getInstanceOwnerIdHash();
        ...
        const { sign } = await import('@refinio/one.core/lib/crypto/sign.js');
        const signature = await sign(new TextEncoder().encode(JSON.stringify(acceptanceVC)), keys.privateSignKey);
        return {
            ...acceptanceVC,
            signature: signature
        };
     ...
     */
    async storeContactWithVC(credential, discoveryVC) {
        ...
        const profile = await ProfileModel.constructWithNewProfile(credential.subject, this.nodeOneCore.ownerId, 'discovered-contact', [endpoint], [credential.publicKey]);
        ...
        // Extend the Person object with trust info
        const someoneData = {
            ...person,
            trustVCs: [vcHash], // Reference to the discovery VC
            trustLevel: this.TRUST_LEVELS.DISCOVERED,
            discoverySource: discoveryVC.discovery.source
        };
        const someone = await SomeoneModel.default.constructWithNewSomeone(someoneData);
     ...
     */
    async canCommunicateWith(personId, operation = 'message') {
     ...
     */
    async getContactTrustVCs(personId) {
    ...
    async acceptContact(personId, options = {}) {
        ...
        const { storeUnversionedObject } = await import('@refinio/one.core/lib/storage-unversioned-objects.js');
        const vcHash = await storeUnversionedObject(acceptanceVC);
     ...
     */
    async getContactTrustLevel(personId) {
        // Get the contact's trust VCs
        const trustVCs = await this.getContactTrustVCs(personId);
        if (!trustVCs || trustVCs.length === 0) {
     ...
     */
    async getContactsByTrustLevel(trustLevel) {
        if (!this.nodeOneCore.leuteModel) {
            return [];
        }
     ...
     * Create a block VC
     */
    async createBlockVC(personId, reason) {
        ...
        // Sign it
        const { sign } = await import('@refinio/one.core/lib/crypto/sign.js');
        const signature = await sign(new TextEncoder().encode(JSON.stringify(blockVC)), keys.privateSignKey);
        return {
            ...blockVC,
            signature: signature
        };
