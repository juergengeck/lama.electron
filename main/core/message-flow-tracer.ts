...
class MessageFlowTracer {
  ...
  tracePoint(messageId: any, point: any, details = {}) {
    ...
    if (trace.length > 0) {
      ...
      traceEntry.deltaMs = currentTime.getTime() - lastTime.getTime()
    }
    ...
    console.log(
      `${color}[TRACE][${String(messageId).substring(0, 8)}] ${point}`,
      details,
      trace.length > 1 ? `(+${traceEntry.deltaMs}ms)` : ''
    )
  }
  
  getColorForPoint(point: string): string {
  ...
  getFullTrace(messageId: any): any {
    if (!this.traces.has(messageId)) {
      return null
    }
  ...
  generateSummary(trace: any): any {
    const steps: any[] = trace.map((t: any) => t.point).join(' → ')
    return steps
  }
  
  logFullTrace(messageId: any): any {
    ...
    fullTrace.points.forEach((point: any, idx: any) => {
