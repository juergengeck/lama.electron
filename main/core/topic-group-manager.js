...
class TopicGroupManager {
    ...
    async getOrCreateConversationGroup(topicId, aiPersonId = null) {
        ...
        try {
            ...
            console.log(`[TopicGroupManager] Persons:`, participants.map((p) => String(p).substring(0, 8)).join(', '));
            // Cache the group
    ...
    async createP2PTopic(topicName, topicId, participantIds) {
        ...
        console.log(`[TopicGroupManager] P2P participants:`, participantIds.map((p) => String(p).substring(0, 8)).join(', '));
    ...
    async createGroupTopic(topicName, topicId, participantIds = [], autoAddChumConnections = false) {
        ...
        const topic = await this.nodeOneCore.topicModel.createGroupTopic(topicName, topicId, this.nodeOneCore.ownerId);
