...
class DirectSocketSpareConnections {
  public spareConnections: any;
  public pendingHandovers: any;
   ...
   */
  registerSpareConnection(publicKey: any, connection: any): any {
    ...
    MessageBus.send('log', `[SpareConnections] Registered spare connection for ${String(key).substring(0, 8)}`)
  }
   ...
   */
  getSpareConnection(publicKey: any): any {
    const key = typeof publicKey === 'string' ? publicKey : Buffer.from(publicKey).toString('hex')
    ...
    if (connections && connections.length > 0) {
      ...
      MessageBus.send('log', `[SpareConnections] Retrieved spare connection for ${String(key).substring(0, 8)}`)
      return conn
    }
   ...
   */
  shouldHandover(targetPublicKey: any): any {
   ...
   */
  async createSpareAfterPairing(url: any, cryptoApi: any, leuteConnectionsModule: any): Promise<any> {
    try {
      ...
      MessageBus.send('log', `[SpareConnections] Ready for CHUM connection from ${String(publicKey).substring(0, 8)}`)
    } catch (error) {
      MessageBus.send('log', `[SpareConnections] Error creating spare: ${(error as Error).message}`)
    }
  }
}
