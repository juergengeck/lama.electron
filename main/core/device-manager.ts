...
class DeviceManager {
  ...
  async registerDevice(deviceInfo: any): Promise<any> {
    ...
    const invite: any = await this.createDeviceInvite(device)
   ...
   */
  async createDeviceInvite(device: any): Promise<any> {
    const inviteOptions = {
    ...
    const invite: any = await (nodeOneCore as any).createLocalInvite(inviteOptions)
   ...
   */
  handleDeviceConnection(ws: any, deviceId: any): any {
   ...
   */
  handleDeviceDisconnection(ws: any): any {
  ...
  getConnectedDevices(): any {
    return Array.from(this.devices.values()).filter((d: any) => d.status === 'connected')
  }

  /**
   * Broadcast to all connected devices
   */
  broadcastToDevices(message: any): any {
   ...
   */
  sendToDevice(deviceId: any, message: any): any {
    for (const [ws, device] of this.connections) {
   ...
   * Remove a device
   */
  async removeDevice(deviceId: any): Promise<any> {
    const device = this.devices.get(deviceId)
  ...
  async loadDevices(): Promise<any> {
    try {
      const data: any = await fs.readFile(this.configFile, 'utf8')
   ...
   */
  getDeviceConfig(deviceId: any): any {
    const device = this.devices.get(deviceId)
