...
class ContentSharingManager {
   ...
   */
  async initializeSharing(browserPersonId) {
   ...
   */
  async grantAccessToExistingContent() {
   ...
   */
  async grantAccessToContacts(createAccess, SET_ACCESS_MODE) {
    ...
    try {
      ...
      for (const someone of others) {
        try {
          ...
          console.warn(`[ContentSharing] Failed to grant access to contact:`, error.message)
        }
      }
   ...
   */
  async grantAccessToChannels(createAccess, SET_ACCESS_MODE) {
    ...
    try {
      ...
      console.log(`[ContentSharing] Found ${channelInfos.length} channels to share`)
      ...
      for (const channelInfo of channelInfos) {
        try {
          ...
          console.warn(`[ContentSharing] Failed to grant access to channel ${channelInfo.id}:`, error.message)
        }
      }
   ...
   */
  async setupNewContentListeners() {
    ...
    if (this.nodeOneCore.leuteModel) {
      ...
      this.nodeOneCore.leuteModel.onSomeoneAdded.listen(async (someoneId) => {
        console.log(`[ContentSharing] New contact added: ${someoneId.substring(0, 8)}...`)
    ...
    if (this.nodeOneCore.channelManager) {
      ...
      this.nodeOneCore.channelManager.onChannelCreated.listen(async (channelInfo) => {
