...
class NodeOneCore implements INodeOneCore {
  ...
  async initOneCoreInstance(username: string, password: string, directory: string): Promise<void> {
    ...
    // Import MultiUser for multi-user support
    const { default: MultiUser } = await import('@refinio/one.models/lib/models/Authenticator/MultiUser.js')
    ...
    console.log('[NodeOneCore] Using MultiUser pattern for Node instance:', email)
    ...
    console.log('[NodeOneCore] Creating MultiUser instance with', allRecipes.length, 'recipes')

    this.oneAuth = new MultiUser({
      directory: oneDbPath,
      recipes: allRecipes,
      reverseMaps: new Map([
        ...(ReverseMapsStable || []),
    ...
    try {
      ...
      const isRegistered = await this.oneAuth.isRegistered(email, instanceName)
      console.log('[NodeOneCore] isRegistered() returned:', isRegistered)

      if (isRegistered) {
        console.log('[NodeOneCore] Instance already registered, logging in...')
        await this.oneAuth.login(email, password, instanceName)
      } else {
        ...
        await this.oneAuth.register(email, password, instanceName)
      }
  ...
  async initializeModels(): Promise<any> {
    ...
    if (!this.topicGroupManager) {
      this.topicGroupManager = new TopicGroupManager(this, {
        ...
        getObjectByIdHash,
        getObject,
        getAllOfType: async (type: string) => [], // Stub implementation - TopicGroupManager doesn't actually use this
        createAccess,
