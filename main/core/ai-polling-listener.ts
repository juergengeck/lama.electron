...
import type { TopicModel, ChannelManager } from '@refinio/one.models/lib/models/index.js'
...
class AIPollingListener {
  ...
  constructor(topicModel: TopicModel, channelManager: ChannelManager, llmManager: LLMManager, aiAssistantModel: AIAssistantModel) {
   ...
   */
  async pollChannels(): Promise<any> {
    try {
      // Get all channels
   ...
   */
  async checkChannelForNewMessages(channelId: any): Promise<any> {
    try {
  ...
  isAIMessage(message: any): any {
    ...
    if (this.aiAssistantModel) {
      ...
      return aiContacts.some((contact: any) => contact.personId === sender)
    }
   ...
   */
  async processUserMessage(topicRoom: any, message: any, channelId: any): Promise<any> {
    ...
    try {
      ...
      const response = await (this.llmManager as any).chat(messages, modelId)
      
      if (response) {
        console.log(`[AIPollingListener] AI response generated: "${String(response).substring(0, 50)}..."`)
