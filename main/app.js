...
class MainApplication {
    ...
    createWindow() {
        ...
        if (process.env.NODE_ENV !== 'production') {
            ...
            // When running from dist/main/, we need to go up two levels to reach project root
            const projectRoot = __dirname.endsWith(path.join('dist', 'main'))
                ? path.join(__dirname, '..', '..')
                : path.join(__dirname, '..');
            this.mainWindow.loadFile(path.join(projectRoot, 'electron-ui', 'dist', 'index.html'));
        }
