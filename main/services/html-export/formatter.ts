 ...
 */
export function createHTMLDocument(content: string, title: string, options: CreateHTMLDocumentOptions = {}): string {
 ...
 */
export function addStyles(theme: 'light' | 'dark' | 'auto'): string {
  const baseStyles = `
    * {
 ...
 */
export function formatMessage(microdata: string, options: FormatMessageOptions = {}): string {
 ...
 */
export function createHeader(metadata: any): string {
  const {
  ...
  const participantsList: any[] = participants.map((p: any) =>
 ...
 */
export function addContentSecurityPolicy(): string {
 ...
 */
function formatTimestamps(microdata: any): any {
  return microdata.replace(
    /<time([^>]*itemprop="timestamp"[^>]*)>([^<]+)<\/time>/g,
    (match: any, attributes: any, isoDate: any) => {
 ...
 */
function addHashDisplay(microdata: any): any {
