...
class LLMManager extends EventEmitter {
    ...
    enhanceMessagesWithTools(messages) {
        ...
        const isWelcomeMessage = messages.some((m) => m.content && (m.content.includes('Generate a welcome message') ||
    ...
    async processToolCalls(response) {
        ...
        try {
            ...
            if (toolCall.tool) {
                ...
                const result = await mcpManager.executeTool(toolCall.tool, toolCall.parameters || {});
    ...
    async chatWithOllama(model, messages, options = {}) {
        ...
        return await chatWithOllama(model.parameters.modelName, messages, {
...
- Provide summary updates for significant conversation developments`;
    ...
    async chatWithLMStudio(model, messages) {
        ...
        if (model.parameters.stream) {
            const stream = lmstudio.streamChatWithLMStudio(model.parameters.modelName, messages, {
                temperature: model.parameters.temperature,
                max_tokens: model.parameters.maxTokens
            });
            let fullResponse = '';
        ...
        // Non-streaming chat
        return await lmstudio.chatWithLMStudio(model.parameters.modelName, messages, {
            temperature: model.parameters.temperature,
            max_tokens: model.parameters.maxTokens
        });
    }
    async chatWithClaude(model, messages) {
        ...
        return await chatWithClaude(model.id.replace('claude:', ''), messages, {
            apiKey,
            temperature: model.parameters.temperature,
            max_tokens: model.parameters.maxTokens
        });
