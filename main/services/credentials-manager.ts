...
class CredentialsManager {
  public trustedCredentials: any;
  public ownCredentials: any;
  public authorityLevels: any;
   ...
   */
  async createSettingsCredential(subject: any, authority: any, settingsScope: any, expiryHours = 24): Promise<unknown> {
   ...
   */
  async validateCredential(credential: any, requestedAction: any, settingsKey: any): Promise<any> {
    try {
   ...
   */
  addTrustedIssuer(instanceId: any, publicKey: any, trustLevel = 'DEVICE_ADMIN') {
   ...
   */
  isTrustedIssuer(instanceId: any): any {
   ...
   */
  getPermittedActions(authority: any, settingsScope: any): any {
    ...
    if (this.hasRequiredAuthority(authority, 'USER')) {
      if (settingsScope.some((s: any) => s.startsWith('settings.appearance') || s.startsWith('settings.notifications'))) {
        actions.add('write')
        actions.add('update')
      }
    }
    ...
    if (this.hasRequiredAuthority(authority, 'DEVICE_ADMIN')) {
      if (settingsScope.some((s: any) => s.startsWith('settings.connections') || s.startsWith('settings.network'))) {
        actions.add('write')
        actions.add('update')
        actions.add('sync')
      }
   ...
   */
  hasRequiredAuthority(currentAuthority: any, requiredAuthority: any): any {
   ...
   */
  async signCredential(credential: any): Promise<any> {
   ...
   */
  async verifyCredentialSignature(credential: any): Promise<any> {
    try {
      const credentialContent = JSON.stringify({
        ...credential,
        proof: undefined
   ...
   */
  async createBootstrapCredentials(): Promise<any> {
