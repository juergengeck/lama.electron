...
import { autoInitialize } from './dist/main/startup/auto-init.js';
import ipcLogger from './dist/main/utils/ipc-logger.js';
...
function createWindow() {
    console.log('[Main] createWindow called, __dirname:', __dirname);
    ...
    // In development, load from built files (preload works properly this way)
    if (process.env.NODE_ENV !== 'production' && false) {
        mainWindow?.loadURL('http://localhost:5174');
        // Workaround: Manually inject electronAPI in dev mode since preload doesn't work with Vite dev server
        mainWindow?.webContents.once('dom-ready', () => {
            console.log('[Main] Injecting electronAPI for dev mode...');
            mainWindow?.webContents.executeJavaScript(`
        window.electronAPI = {
          platform: '${process.platform}',
          isElectron: true,
          log: (msg) => console.log('[electronAPI]', msg),
          invoke: (...args) => window.electron.ipcRenderer.invoke(...args),
          on: (channel, callback) => {
            const sub = (event, ...args) => callback(...args);
            window.electron.ipcRenderer.on(channel, sub);
            return () => window.electron.ipcRenderer.removeListener(channel, sub);
          },
          off: (channel, callback) => window.electron.ipcRenderer.removeListener(channel, callback),
          createUdpSocket: (opts) => window.electron.ipcRenderer.invoke('create-udp-socket', opts),
          clearAppData: () => window.electron.ipcRenderer.invoke('app:clearData'),
          createInvitation: () => window.electron.ipcRenderer.invoke('invitation:create'),
          getConnectionsInfo: () => window.electron.ipcRenderer.invoke('connections:info'),
          getConnectionsStatus: () => window.electron.ipcRenderer.invoke('connections:status'),
          getDevices: () => window.electron.ipcRenderer.invoke('devices:list'),
          getConnectedDevices: () => window.electron.ipcRenderer.invoke('devices:connected'),
          registerDevice: (info) => window.electron.ipcRenderer.invoke('devices:register', info),
          removeDevice: (id) => window.electron.ipcRenderer.invoke('devices:remove', id),
          getInstanceInfo: () => window.electron.ipcRenderer.invoke('instance:info'),
        };
        console.log('[Main] ✅ electronAPI injected');
      `);
        });
        ...
        // If running from dist/, go up one level to find electron-ui
        const uiPath = __dirname.endsWith('dist')
            ? path.join(__dirname, '..', 'electron-ui', 'dist', 'index.html')
            : path.join(__dirname, 'electron-ui', 'dist', 'index.html');
        console.log('[Main] Loading UI from:', uiPath);
        console.log('[Main] __dirname:', __dirname);
        console.log('[Main] File exists:', require('fs').existsSync(uiPath));
...
app.whenReady().then(async () => {
    ...
    // Start Vite server first if in development (disabled - using built files for preload to work)
    if (process.env.NODE_ENV !== 'production' && false) {
...
ipcMain.handle('app:clearData', async (event) => {
    ...
    try {
        ...
        try {
            const { default: nodeOneCore } = await import('./dist/main/core/node-one-core.js');
            if (nodeOneCore) {
            ...
            const { default: nodeProvisioning } = await import('./dist/main/services/node-provisioning.js');
        ...
        try {
            const { default: stateManager } = await import('./dist/main/state/manager.js');
        ...
        const modulePathsToDelete = [
            './dist/main/core/node-one-core.js',
            './dist/main/services/node-provisioning.js',
            './dist/main/state/manager.js',
            './dist/main/app.js'
        ];
