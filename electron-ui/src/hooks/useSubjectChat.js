 ...
 */
export function useSubjectChat(conversationId, currentUserId, llmContactId) {
    const [messages, setMessages] = useState([]);
    const [mediaItems, setMediaItems] = useState([]);
    const [llmIdentity, setLlmIdentity] = useState(null);
     ...
     */
    const extractMessageSubjects = useCallback((content) => {
    ...
    const processMessage = useCallback(async (message, senderId) => {
        ...
        if (message.attachments) {
            for (const attachment of message.attachments) {
                ...
                for (const subject of subjects) {
                    await subjectService.attachSubject(subject, attachment.hash, senderId, 0.8, // High confidence for explicit message Subjects
                    conversationId);
                }
                // Predict additional Subjects
                const predicted = await subjectService.predictSubjects(attachment.mimeType || 'unknown', conversationId, senderId);
                for (const pred of predicted.slice(0, 3)) {
                    if (pred.confidence > 0.6) {
                        await subjectService.attachSubject(pred.name, attachment.hash, senderId, pred.confidence, conversationId);
                    }
                }
            }
        }
     ...
     */
    const buildLLMContext = useCallback(async (recentMessages, llmId) => {
        const identity = subjectService.getSubjectSignature(llmId);
        const contextSubjects = new Set();
     ...
     */
    const processLLMResponse = useCallback(async (response, llmId) => {
     ...
     */
    const addMessageSubject = useCallback(async (messageId, subject) => {
    ...
    useEffect(() => {
        const loadContextSubjects = async () => {
            ...
            const conversationSubjects = new Map();
