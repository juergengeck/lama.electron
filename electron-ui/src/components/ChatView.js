...
export function ChatView({ conversationId = 'default', onProcessingChange }) {
            ...
            try {
                ...
                if (conversationId === 'default' || conversationId === 'ai-chat') {
                    ...
                    if (messages.length > 0) {
                        const aiMessage = messages.find(m => m.sender?.toLowerCase().includes('ai') ||
    ...
    return (_jsxs(Card, { className: "h-full flex flex-col", children: [_jsx(CardHeader, { className: "border-b", children: _jsx("div", { className: "flex items-center justify-between", children: _jsxs("div", { className: "flex items-center space-x-2", children: [_jsx(MessageSquare, { className: "h-5 w-5 text-primary" }), _jsx(CardTitle, { children: conversationName })] }) }) }), _jsx(CardContent, { className: "flex-1 p-0 min-h-0", children: _jsx(MessageView, { messages: messages, currentUserId: user?.id, onSendMessage: handleSendMessage, placeholder: "Type a message...", showSender: true, loading: loading, isAIProcessing: isAIProcessing, aiStreamingContent: aiStreamingContent }) })] }));
}
