...
export class ShellMCPServer {
    ...
    async runBackgroundTask(command, taskId, cwd) {
    ...
    async checkBackgroundTask(taskId) {
    ...
    async killBackgroundTask(taskId) {
        const task = this.runningProcesses.get(taskId);
        if (!task) {
            return {
                content: [
    ...
    async getEnvironment(variables) {
