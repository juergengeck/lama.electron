...
export class LamaMCPServer {
  ...
  private appModel: any // Will be injected
  ...
  private async sendMessage(topicId: string, message: string) {
    try {
  ...
  private async getMessages(topicId: string, limit = 10) {
    try {
  ...
  private async getSettings(category?: string) {
  ...
  private async updateSetting(category: string, key: string, value: any) {
