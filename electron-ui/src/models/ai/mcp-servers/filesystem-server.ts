...
export class FilesystemMCPServer {
  ...
  constructor(basePath: string = '/Users/gecko/src/lama.electron') {
  ...
  private async readFile(filePath: string, encoding = 'utf8') {
  ...
  private async searchFiles(pattern: string, basePath = '.') {
  ...
  private async grepFiles(pattern: string, searchPath: string, filePattern = '*') {
  ...
  private async editFile(filePath: string, search: string, replace: string, all = false) {
    const fullPath = path.resolve(this.basePath, filePath)
