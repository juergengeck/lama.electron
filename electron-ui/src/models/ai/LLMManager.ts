...
import type { Person, OneObjectTypes, OneObjectTypeNames, CLOB } from '@refinio/one.core/lib/recipes'
...
import { chatWithOllama, generateWithOllama, testOllamaModel } from '../../services/ollama'
import { chatWithClaude, claudeService, CLAUDE_MODELS } from '../../services/claude'
import { MCPManager } from './MCPManager'

// Import types from LAMA recipes
import type { LLM, LLMSettings } from '../../types/@OneObjectInterfaces'

// Extend the base LLM type for runtime use (non-persistent fields)
export interface RuntimeLLM extends LLM {
  // Runtime-only fields that don't get persisted
...
// Default settings
const DEFAULT_LLM_SETTINGS: Partial<LLMSettings> = {
  temperature: 0.7,
  maxTokens: 2048,
...
const DEFAULT_MODELS: RuntimeLLM[] = [
  {
    $type$: 'LLM',
...
export class LLMManager {
  ...
  public readonly onModelLoaded = new OEvent<(modelId: string) => void>()
  public readonly onModelUnloaded = new OEvent<(modelId: string) => void>()
  public readonly onChatResponse = new OEvent<(response: string) => void>()
  public readonly onChatStream = new OEvent<(data: { chunk: string, isThinking?: boolean, partial: string }) => void>()
  public readonly onError = new OEvent<(error: Error) => void>()
  
  private models: Map<string, LLM> = new Map()
  private modelSettings: Map<string, LLMSettings> = new Map()
  private mcpManager?: MCPManager
  private isInitialized = false
  private messageBus?: MessageBusObj
  private modelStorageChannel?: string
   ...
   */
  private async getOrCreateModelStorageChannel(): Promise<string> {
   ...
   */
  private async registerOllamaModels(): Promise<void> {
    try {
      // Register GPT-OSS model
      const gptOssModel: LLM = {
        $type$: 'LLM',
        $v$: 1,
        id: 'ollama:gpt-oss',
        name: 'GPT-OSS',
   ...
   */
  getModelsByCapability(capability: string): LLM[] {
   ...
   */
  getBestModelForTask(task: 'coding' | 'reasoning' | 'chat' | 'analysis'): LLM | null {
   ...
   * Get model settings
   */
  getModelSettings(modelId: string): LLMSettings | undefined {
   ...
   */
  getAllModelSettings(): LLMSettings[] {
   ...
   */
  getModel(id: string): LLM | undefined {
   ...
   */
  setMCPManager(mcpManager: any): void {
   ...
   */
  private async saveModelSettings(settings: LLMSettings): Promise<void> {
    // Store in memory
   ...
   * Add a new model
   */
  async addModel(llm: LLM): Promise<void> {
    try {
      // Store model locally
      this.models.set(llm.id, llm)
   ...
   */
  async updateModelSettings(modelId: string, updates: Partial<Omit<LLMSettings, '$type$' | '$v$' | 'llmId'>>): Promise<void> {
    let settings = this.modelSettings.get(modelId)
   ...
   */
  private async getClaudeApiKey(): Promise<string | null> {
   ...
   * Test Claude API key
   */
  async testClaudeApiKey(apiKey: string): Promise<boolean> {
    try {
      claudeService.initialize({ apiKey })
   ...
   */
  private enhanceMessagesWithTools(messages: ChatMessage[]): ChatMessage[] {
  ...
  getLoadedModels(): LLM[] {
    ...
    return loadedModelIds
      ...
      .filter((model): model is LLM => model !== undefined)
  }
   ...
   */
  getModelByFilePath(filePath: string): LLM | undefined {
    return Array.from(this.models.values())
   ...
   */
  getModelProvider(modelId: string): { provider: string; type: string } | undefined {
    const model = this.models.get(modelId)
