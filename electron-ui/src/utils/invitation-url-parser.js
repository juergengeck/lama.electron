 ...
 */
export function parseInvitationUrl(invitationLink) {
    try {
 ...
 */
function getPairingInformation(invitationLink) {
    try {
        const invitation = JSON.parse(decodeURIComponent(invitationLink.split('#')[1]));
 ...
 */
export function extractInvitationFromHash(url) {
 ...
 */
export function isValidInvitationUrl(url) {
    const result = parseInvitationUrl(url);
 ...
 */
export function getInvitationMode(url) {
